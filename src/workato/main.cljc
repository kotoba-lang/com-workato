(ns workato.main
  "Kotodama WASM entrypoint for the Workato clean-room actor (L4) — Clojure port.
  ...
  facts (`workato.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "workato")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"workato.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "workato-compat" :tier tier :entities entities} 200])
...
(defn start! [] :workato-compat/ready)
